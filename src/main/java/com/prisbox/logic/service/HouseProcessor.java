...
public class HouseProcessor implements PageProcessor {
	...
	public void process(Page page) {
		...
		/*
		 * if (page.getUrl().regex(URL_LIST).match()) { if (flag) { List<String>
		 * list = new ArrayList<String>(99); for (int i = 2; i < 101; i++) {
		 * list.add(PREFIX + "/pg" + i); System.out.println(list.get(i - 2)); }
		 * page.addTargetRequests(list); flag = false; } //
		 * page.addTargetRequests(page.getHtml().xpath(
		 * "//div[@class='sellListContent']").links().regex(URL_POST).all());
		 * List<String> houses = page.getHtml().xpath(
		 * "//div[@class='content']//div[@class='leftContent']/ul").links()
		 * .regex(URL_POST).all(); System.out.println(houses);
